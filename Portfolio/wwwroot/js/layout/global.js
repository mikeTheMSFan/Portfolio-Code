(function ($) {
  ...
  function toggleSlideBar() {
    if (sliderbar.hasClass('closed')) sliderbar.addClass('opened').removeClass('closed');
    else sliderbar.addClass('closed').removeClass('opened');
  }
  ...
  $('.po-list').find('.on-queue').hide();
  $('.po-btn .load-btn').on('click', function (e) {
    $(this)
      .html("<div class='loader-icon-2'></div>")
      .delay(1000)
      .queue(function (next) {
        ...
        $('.po-list')
          .find('.load-item')
          .fadeToggle(0, 'swing', function () {
            grid_isotope.isotope('layout');
          });

        const projectIds = getFourProjectIds();
        if (projectIds.length) {
          const projects = document.querySelectorAll(projectIds);
          appendLoadItemClass(projects);
        } else {
